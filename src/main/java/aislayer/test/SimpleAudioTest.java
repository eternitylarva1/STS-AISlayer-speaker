...
public class SimpleAudioTest {
    ...
    private static void testSupportedFormats() {
        System.out.println("=== 测试系统支持的音频格式 ===");
        ...
        for (AudioFormat format : formats) {
            ...
            System.out.println("格式支持测试: " + format + " -> " + (supported ? "支持" : "不支持"));
        }
    }
    ...
    private static File createTestAudioFile() {
        try {
            ...
            System.out.println("测试音频文件创建成功: " + testFile.getAbsolutePath());
            return testFile;
            
        } catch (Exception e) {
            System.err.println("创建测试音频文件失败: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    private static void testPlayAudioFile(File audioFile) {
        ...
        try {
            ...
            System.err.println("音频播放测试失败: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public static void testMP3Support() {
        System.out.println("=== 测试MP3文件支持 ===");
        
        try {
            // 列出所有支持的音频文件类型
            AudioFileFormat.Type[] types = AudioSystem.getAudioFileTypes();
            for (AudioFileFormat.Type type : types) {
                System.out.println("支持的音频类型: " + type.getExtension());
            }
            ...
            System.out.println("MP3读取支持: " + (mp3ReadingSupported ? "是" : "否"));
            System.out.println("MP3写入支持: " + (mp3WritingSupported ? "是" : "否"));
            
        } catch (Exception e) {
            System.err.println("测试MP3支持失败: " + e.getMessage());
            e.printStackTrace();
        }
    }
